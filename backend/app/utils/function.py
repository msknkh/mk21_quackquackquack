...
from app.models import Atms, BankMitras, Banks, PostOffices, CSCs,AtmRequests_by_pincode

fin_touch_pt_1=['atms','bankMitras']
fin_touch_pt_2=['banks','postoffices','cscs']
...
def create_pincode_data(lat,longi,pin_code,distance):
    data = AtmRequests_by_pincode.query.filter_by(pin_code=pin_code).first()

    if data is None:
        new_entr=AtmRequests_by_pincode(pin_code=pin_code,number_of_requests=1,avg_dist=distance)
        db.session.add(new_entr)
    else:
	    data.number_of_requests=data.number_of_requests+1
	    data.avg_dist=(data.avg_dist*data.number_of_requests+distance)/(data.number_of_requests+1)

    db.session.commit()

    print('hello')

    return 'Pincode Data Updated'
